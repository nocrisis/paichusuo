...
//时分秒的有效检测
    Dates.timeVoid = function (times, index) {
        ...
        } else if (Dates.ymd[0] == Dates.mins[0] && Dates.ymd[1] + 1 == Dates.maxs[1] && Dates.ymd[2] == Dates.maxs[2]) {
...
//事件
    Dates.events = function () {
        ...
        Dates.each(S('#laydate_table td'), function (i, elem) {
            Dates.on(elem, 'click', function (ev) {
                // 只有点击确认按钮才能确定时间，关闭时间弹窗
                if (Dates.options.issure == 1) {
                    Dates.viewDate(this.getAttribute('y') | 0, (this.getAttribute('m') - 1) | 0, this.getAttribute('d') | 0, Dates.ymd[3]);
                    return false;
                }
                if (!Dates.hasClass(this, as[1])) {
                    Dates.stopmp(ev);
                ...
                if (set) {
                    ...
                    Dates.on(elem, 'click', function (ev) {
                        if (!Dates.hasClass(this, as[1])) {
                            ...
                            Dates.each(new Array(3), function (i) {
                                Dates.hmsin[i].value = Dates.digit(Dates.timeVoid(Dates.hmsin[i].value, i) ? Dates.mins[i + 3] | 0 : Dates.hmsin[i].value | 0);
                            });
